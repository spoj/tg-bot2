...
- **Agent protocol** (workspace files): `.tg-bot/events.jsonl` is the chat event log; `.tg-bot/outbox/*.json` are Telegram sends; every dispatch is archived to `.tg-bot/sent.jsonl` or `.tg-bot/failed.jsonl`; `.tg-bot/schedules.json` holds reminders; `.tg-bot/task/*.txt|md` files spawn background subagents whose runs live under `.pi/subagents/<id>/` (session, output.md, result.json) and settle as `subagent` events. Sessions persist under `.pi/sessions/*.jsonl`, resume within a two-hour inactivity window, and restart fresh when the agent touches `.tg-bot/new-session`. Model and thinking level are chosen by editing `.pi/agent/settings.json`.
