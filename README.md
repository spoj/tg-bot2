...
If another assembled request arrives while Pi is running, it is passed through Pi's native steering mechanism as one message; the already-active run sends the eventual response, avoiding a reply per quick update. `/new` first flushes that chat's current buffer, waits behind the active run, then disposes the active Pi session and starts a new JSONL file. Older files remain in the workspace and are searchable by Pi tools. During shutdown, Telegram polling stops before ingress admission closes; already accepted batches are drained before AgentManager shutdown so their responses are not suppressed.
Outbound replies, assistant progress, scheduled text, and outbox files share one per-chat FIFO delivery queue: sends for the same chat retain production order while different chats can send concurrently. Shutdown stops all producers first, then drains accepted outbound sends before exiting; a failed send does not prevent later sends from running.
