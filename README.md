...
- **Schedules and multimodal work**: `.schedules.json` rows require an owning conversation. When due, the schedule wakes its owner, which acts directly or delegates with `spawn` or `steer_conversation`. `ask_multimodal` (`extensions/multimodal.ts`) analyzes mixed media through an explicitly selected model; agents use `annotate` to preserve a short content description at the attachment's original timeline position.
