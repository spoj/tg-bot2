...
export function validateRequest(value: unknown): WorkspaceOutboxRequest {
  ...
  if (typeof request.id !== "string" || request.id.length === 0 || request.id.length > MAX_REQUEST_ID_LENGTH) {
    throw new Error(`Outbox request id must be a non-empty string of at most ${MAX_REQUEST_ID_LENGTH} characters`);
  }
  ...
  return { ...request, version: 1, id: request.id, type: request.type } as WorkspaceOutboxRequest;
}
